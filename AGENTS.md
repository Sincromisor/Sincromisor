...
- 再現手順と確認結果はタスク文書に残す。
- ソースコード内のコメントは、未来の保守者に対して「このコードを安全に変更するために必要だが、コードだけでは分からない情報」を残すものである。TypeScript の詳細基準は `documents/rules/coding-ts.md` の「ソースコードコメント品質」を正本とする。
...
取得対象、除外理由、下流処理との契約、例外条件を説明しています。
...
単なる分岐説明ではなく、センサー欠損時の設計意図と視覚品質上の理由を記録しています。
