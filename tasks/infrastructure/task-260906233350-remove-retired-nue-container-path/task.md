...
- [ ] サンプル設定、README、Compose、初期化処理がNeMoだけを現行対応として扱う。
- [ ] `nue` や未対応値で、黙ってNeMo起動やモデル取得・依存導入へ進まず、対応外と分かる失敗になる。
- [ ] Nue用Dockerfileと現役Dockerfileの不要なNueメタデータCOPYを整理し、NeMoのビルドが成功する。
- [ ] 正常なNeMo指定と未対応値1件で導線を確認する。
