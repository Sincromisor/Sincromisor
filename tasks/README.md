...
- 説明文、見出し、表の列名、タスク名は一般的な日本語で書く。コマンド名、ファイル名、設定キー、
  識別子、規格名、固有の状態値など、正確な原表記が必要な語だけを英語で残す。技術用語や検索性だけを英語表記の理由にしない。詳細は `documents/rules/coding-md.md` を正本とする。
...
通常タスクは、親 Codex が現在のワークツリーで `task.md` に沿って実装し、変更範囲に対応する確認を行えばよい。`review.md` と `eval.md` はユーザーが独立確認を求めた場合、または高リスク変更で独立確認を行った場合だけ記録する。`impl.md` は設計判断、仕様からの逸脱、未実行の確認、残リスクがある場合だけ簡潔に使い、コミット内容を複製しない。
...
性能値、負荷・長時間試験、ブラウザー・端末の組み合わせ、網羅率、本番相当環境は、実測した基準値、外部制約、再現済み問題、またはユーザーの明示要求がある場合だけ必須にする。それ以外は開発環境で主要経路が一度動作することを既定の完了条件とする。

外部境界がある場合だけ、設定値・ポート・アドレス・イメージの実行コマンド・死活確認の供給元、消費先、正本を特定する。複数の妥当な方式が残り、公開契約や責務を変える場合だけ先に設計判断を確定する。
...
2026-08-24 以降に作成するタスク名は、内容を説明する日本語を含める。`tasks:new`、`tasks:set`、`tasks:check` がこの境界を検査し、それ以前の英語だけのタスク名は履歴として変更しない。
...
- 高リスクな実環境確認の失敗直後は、コマンド、対象コミット、入力、時刻、終了コード、関連ログ、プロセス・ネットワーク・リソース状態など、
  後片付けや環境復旧で失われる証拠と、その状態でしかできない初期切り分けを先に完了する。共有サービスなどを
  停止した場合は、これらの完了後、長いオフライン調査より先に復旧して他作業への影響を止める。
- 今回の差分が原因なら、直接原因と修正対象を特定し、最小の再現確認、修正、失敗した確認の順で再検証する。
  タスクの前提や受け入れ条件が誤っていた場合は、実装を続けずタスク改訂へ戻す。
...
- 今回の差分が生んだ原因修正を別タスクへ移す場合は、再現手順、証拠、特定済み原因、移管理由、後続タスクIDを残し、
...
| 区分         | 対象                                                                   | 標準経路                                                                         |
| ------------ | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 通常変更     | 単一領域の局所変更                                                     | 現在のワークツリーで直接実装、対象確認、タスク状態と索引を含む1コミット          |
| 統合変更     | 複数コンポーネント、設定、文書の同期                                   | 必要な結合確認。並行作業や未コミット変更との分離が必要な場合だけ専用ワークツリー |
| 高リスク変更 | セキュリティ、データ損失、公開通信契約、本番切替、複数所有者の状態機械 | 専用ワークツリー、独立レビュー・評価、全体ゲートと必要な実環境確認               |
...
3. 通常変更は親が現在のワークツリーで直接実装する。統合変更は必要な場合だけ、高リスク変更は原則として実装ワークツリーを使う。
4. 変更範囲に対応する最小の確認を行う。全体ゲートは高リスク変更、または変更が横断的で全体回帰を検出できる場合だけ実行する。
5. 明示要求または高リスク時だけ task-reviewer / impl-evaluator を呼ぶ。
6. 通常変更は実装・タスク状態・索引を1コミットにまとめる。ワークツリーを使った変更は既存のマージ、`tasks:close`、`tasks:reindex`、ワークツリー削除で完了する。
...
高リスク変更だけは、`tasks/AUTHORING-CHECKLIST.md` の追加確認、独立レビュー・評価、全体ゲートを検討する。
RTC 第 3 段階で使った個別の所有権表や指標表を、無関係なタスクへ一律に要求しない。

コメント品質は `documents/rules/source-comments.md` と対象言語規約を直接適用する。本番コードを変更した場合は、変更したシンボル・処理群・判断と直接の変更理解範囲を全件点検し、必須コメントの欠落・説明不足・陳腐化したコメントを解消するまで完了しない。既存コード由来でも変更理解範囲内なら今回の対象とする。`task.md`、`impl.md`、`eval.md` に別のコメント監査台帳は作らず、最終報告に `コメント点検: PASS` と記す。
...
`build`, `test` を実行し、`check` にリポジトリ全体の Markdown 検査を含める。このため、通常変更の完了判定には使わない。全体ゲートが変更範囲外の既存不整合で失敗した場合は、該当パスを警告として報告し、今回の差分が悪化させていないことを対象確認で示せばよい。
...
タスク管理のみの変更ではタスク管理ツールの検査を実行する。Markdownは変更ファイルだけをPrettierで整形・確認する。リポジトリ全体のMarkdown検査は任意の健全性確認とし、変更範囲外の失敗でタスクを止めない。
