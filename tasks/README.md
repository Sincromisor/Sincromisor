...
起票者は現在の HEAD と依存を確認してから `task.md` を書く。外部境界がある場合は、設定値・port・address・image command・healthcheck の供給元、消費先、正本を特定する。複数の妥当な方式が残るなら実装タスクを起票せず、先に設計判断を確定する。

高リスク統合タスクは、起票時の独立レビューを原則とする。既存の正本から一意に補える不足は `AUTO_FIX` として起票者が反映し、レビューは `APPROVED` とする。公開契約・責務・受け入れ条件を変える不足、または外部設定の供給方式など複数の選択肢を残す不足だけを `NEEDS_REVISION` とする。
...
着手確認で見つかった不足も同じ基準で扱う。既存の正本から一意に決まり、公開契約・責務・受け入れ条件を変えない `AUTO_FIX` は実装を継続し、根拠と補完内容を `impl.md` に残す。複数の妥当な選択肢、公開設定・外部境界の供給方式、責務分担が必要な場合だけ task 改訂へ戻す。
