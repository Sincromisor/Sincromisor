...
この文書は、Sincromisor のタスク管理、Codex サブエージェントの成果物、検証ログ、コミット単位の運用手順である。
...
- 説明文、見出し、表の列名、タスク名は一般的な日本語で書く。コマンド名、ファイル名、設定キー、
  識別子、規格名、固有の状態値など、正確な原表記が必要な語だけを英語で残す。技術用語や検索性だけを英語表記の理由にしない。詳細は `documents/rules/coding-md.md` に従う。
...
- 過去のコミットは形式の前例にせず、この節の規則と例に従う。初回は `npm run git:hooks:install` で検査用フックを有効にし、コミット後も `npm run commit:check` が成功することを確認する。
...
| `build`    | ビルド、依存、Docker Compose、パッケージ周辺 |
| `ci`       | CI、自動化                                   |
| `perf`     | 性能改善                                     |
| `revert`   | 変更の取り消し                               |
...
| `docs`      | 設計文書、ルール文書               |
| `compose`   | Docker Compose / Consul / 保存領域 |
...
BREAKING CHANGE: RTCSignalingServerのoffer応答から...を削除した。
Refs: task-260601153000-example
```
...
| `meta.yaml`   | 親 Codex                     | 作業状態などの管理情報。`tasks:set` で更新               |
...
外部境界がある場合だけ、設定値・ポート・アドレス・イメージの実行コマンド・死活確認の供給元、消費先、定義を管理する場所を特定する。複数の妥当な方式が残り、公開契約や責務を変える場合だけ先に設計判断を確定する。

高リスク変更は、起票時の独立レビューを原則とする。レビューは不足を追加するだけでなく、根拠のない要件を削り、個人開発で実行可能な最小単位へ縮小する。既存の仕様・規約から一意に補える不足は `AUTO_FIX` として反映し、公開契約・責務・利用者が求める結果を変える選択が必要な場合だけ `NEEDS_REVISION` とする。
...
変更前からある不整合、変更範囲外の検査失敗、任意確認を実行できないことは警告として報告し、今回の変更が悪化させていなければタスクを完了できる。変更した文書の整形など、安全かつ一意に直せる不整合は自動修正する。ユーザーの未コミット変更や変更範囲外のファイルは無断で修正しない。
...
- 今回の差分による原因未特定の必須失敗が残る間は評価担当を`PASS`にせず、タスクを`done`へしない。
  継続可能なら`open`、外部要因や権限待ちなら原因と解除条件を記録して`blocked`にする。
- `tasks:close verdict=FAIL`は、原因と証拠を記録した現在試行を`open`のまま区切るためにだけ使う。
  原因調査を省略してFAILを記録する手段ではない。
- 今回の差分が生んだ原因修正を別タスクへ移す場合は、再現手順、証拠、特定済み原因、移管理由、後続タスクIDを残し、
  ユーザーが移管を了承した場合に限る。原因不明のまま後続タスクへ送って元タスクを完了しない。
...
通常変更では `tasks:set status=done verdict=PASS attempts=1` と `tasks:index` を実行し、実装、タスク記録、索引を同じコミットに含める。専用の終了・索引再生成コミットは作らない。
...
2. 既存の仕様・規約から一意に決まる不足は自律補完し、根拠のない要件は削る。公開契約、責務、利用者が求める結果を変える選択だけタスク改訂へ戻す。
...
着手確認で見つかった不足も同じ基準で扱う。既存の仕様・規約から一意に決まり、公開契約・責務・利用者が求める結果を変えない `AUTO_FIX` は実装を継続する。通常変更では最終報告へ記し、別記録が必要な場合だけ `impl.md` を使う。
...
通常の局所変更には、変更ファイルの整形、対象を絞った静的検査・型確認・テストだけを適用する。
...
Biomeの `warn` はコードを見直すための助言であり、警告があるだけで必須確認の失敗や残リスクとは扱わない。変更箇所で新規・増加した警告は内容を確認し、理解や変更のしやすさを改善できる場合に整理する。数値を下げるだけの関数分割や、警告を消すためだけの抑制・閾値変更は行わない。

具体的な不具合、契約違反、保守上の支障などがなければ、警告の有無・件数や許容理由をコミット本文、最終報告、タスク成果物へ毎回記載する必要はない。既存警告の再掲や、警告だけを理由とする別タスクの起票も不要とする。報告が必要な場合は「警告が出た」ではなく、具体的な影響と必要な対応を書く。ユーザーが警告の調査を求めた場合は、その結果を報告する。

検査を失敗させるエラーや、警告の調査で判明した必須要件違反は従来どおり扱う。検査結果を偽って警告ゼロと報告しない。

### 3 点ゲート

3点ゲートは高リスク変更、または変更が横断的で対象確認だけでは回帰を検出できない場合に使う。通常変更では変更ファイルを整形し、対象を絞った静的検査・型確認・テストだけを実行する。
...
| ゲート              | 実装者                                                        | 評価者                                               |
| ------------------- | ------------------------------------------------------------- | ---------------------------------------------------- |
| 静的検査 / 整形     | `npm run check`, `uv run ruff check .` など                   | `npm run check`, `uv run ruff format --check .` など |
| 型チェック / ビルド | `npm run build`, `uv run --group dev --group full ty check .` | 同左。差分を生まないビルド / 型検査を使う            |
| テスト              | 変更範囲の `npm run test`, `uv run pytest`                    | 同左。受け入れ条件を満たすか批判的に確認する         |
