...
既存のstatuses、health、metrics、browser UIだけを観測点に使う。実装不具合や観測点不足が見つかった場合は本taskへ
抱え込まず、根本原因、移行との関係、最小受け入れ条件を持つ小さなPhase 4 taskを起票する。移行必須条件を観測できる
production相当smoke手順が利用可能になった時点で、runbookを最初から再実行する。過去artifactと判定履歴は書き換えない。
