...
- [ ] Pionを停止してaiortcを復旧し、同じChrome経路で1回のsmoke testをFrontendと下流serviceのrebuildなしに完了する。
...
既存のstatuses、health、metrics、browser UIだけを観測点に使う。Pionとrollback後のaiortcで各1回実行したら判定し、
追加の前提監査、Docker挙動調査、browser matrix、新しいoracleは行わない。実装不具合で移行必須条件を満たせない場合だけ、
