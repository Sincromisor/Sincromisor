...
- [ ] `MotionReplayPlayer` の replay result から metrics summary を生成でき、`motion-debug` window API で `calculateReplayMetrics()` が summary を返す。
...
- `addedLatencyMs` は Phase 3 の FrameClock 前は `timestamp` slot から取れる範囲だけ計算し、不足時は `not_available` にする。
...
- `SincroTrackerWorkerStats` は worker mode、transfer / round trip、droppedFrames を持つ（`sincromisor-frontend/src/features/gaze/trackingRuntime/sincroTrackerWorkerTypes.ts:14`）。`addedLatencyMs` の初期入力候補として使う。
...
- synthetic fixture で次を検証する:
    ...
    - constraint reason がある frame の occupancy が frame count 比で計算される。
