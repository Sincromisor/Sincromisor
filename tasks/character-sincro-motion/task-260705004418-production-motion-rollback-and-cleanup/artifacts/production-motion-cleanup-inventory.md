...
| ----------------------------------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `composerArmApplicationMode`                                      | temporary rollback flag                 | Debug Console 限定。通常設定 UI / URL query / env / backend API / 保存設定 contract には出していない。          | 残置                      | motion runtime                 | full application が常時有効化され、arm stage へ戻す runbook が不要になり、P0 replay と複数 VRM 実機確認が継続 PASS。          | `sincroPoseRetargetTypes.ts` TSDoc、`motion.md`、runtime ownership map |
...
| `fullNormalizedPoseApplicationMode`                               | temporary rollback flag                 | Debug Console 限定。既定 `"off"` は段階別 path、`"upper_body"` は current available result だけ full apply。    | 残置                      | motion runtime                 | full application を常時有効化しても P0 replay、camera degradation / recovery、chat / sincro mode 切替、複数 VRM が継続 PASS。 | `sincroPoseRetargetTypes.ts` TSDoc、`motion.md`、runtime ownership map |
...
| old direct write fallback path                                    | production fallback                     | arm / torso は rollback hook として残す。full unavailable では staged writer 前に identity clear してから戻る。 | 残置                      | motion runtime                 | full application 常時有効化後、direct writer へ戻す復旧手順を廃止できる時。                                                   | runtime ownership map、rollback runbook                                |
...
| -------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| full application 常時有効化            | 現在の default は `"off"`。通常設定 contract に広げないまま developer flag で確認する段階。 | P0 replay / real VRM visual artifact を揃えて default 切替可否を判断する task。                  |
| staged rollback flag 削除              | runbook でまだ復旧 hook として使う。                                                        | full application 常時有効化後、一定期間の metrics / visual PASS を確認して削除 task を起票する。 |
