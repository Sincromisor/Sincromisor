...
async function collectGeneratedOrphans(outputs) {
    ...
    try {
        for (const entry of await readdir(codexAgentsDir, { withFileTypes: true })) {
            if (entry.isFile() && entry.name.endsWith(".toml")) {
                candidates.push(join(codexAgentsDir, entry.name));
            }
        }
    } catch {
        // ディレクトリが無ければ orphan なし
    }
    ...
    for (const candidate of candidates) {
        ...
        if (
            content.includes("generated by scripts/gen/genCodex.mjs") ||
            content.includes("scripts/gen/genCodex.mjs が .claude/")
        ) {
...
async function main() {
    ...
    if (CHECK) {
        ...
        const orphans = await collectGeneratedOrphans(outputs);
        if (stale.length || orphans.length) {
            const parts = [];
            if (stale.length) {
                parts.push(
                    `次の生成物が最新ではありません（\`gen:codex\` を実行して下さい）:\n` +
                        stale.map((p) => `  - ${p}`).join("\n"),
                );
            }
            if (orphans.length) {
                parts.push(
                    `次の生成物は .claude/ に対応する定義がありません（削除して下さい）:\n` +
