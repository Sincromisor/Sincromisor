...
// BuildPion は検査済み Go binary だけを使って cmd/pion-poc を build する。
//
...
func (e Environment) BuildPion(ctx context.Context, outputPath string) ([]byte, error) {
	...
	command := e.GoCommand(ctx, "build", "-trimpath", "-o", outputPath, "./cmd/pion-poc")
	output, err := command.CombinedOutput()
	if err != nil {
		return output, fmt.Errorf("build pion-poc: %w", err)
	}
	return output, nil
}
