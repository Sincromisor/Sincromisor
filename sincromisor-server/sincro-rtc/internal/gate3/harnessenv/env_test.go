...
func TestValidatedGoPathBuildsAndRerunsEvidence(t *testing.T) {
	...
	output := filepath.Join(t.TempDir(), "sincro-rtc")
	if _, err := loaded.BuildPion(context.Background(), output); err != nil {
		t.Fatalf("BuildPion() error = %v", err)
	}
	command := loaded.GoCommand(context.Background(), "test", "-run", "HarnessContract", "./cmd/sincro-rtc")
	...
	if len(lines) != 2 ||
		lines[0] != "build -trimpath -o "+output+" ./cmd/sincro-rtc" ||
		lines[1] != "test -run HarnessContract ./cmd/sincro-rtc" {
...
func makeEnvironmentFixture(t *testing.T) (string, map[string]string) {
	...
	moduleRoot := filepath.Join(repository, "sincromisor-server", "sincro-rtc")
