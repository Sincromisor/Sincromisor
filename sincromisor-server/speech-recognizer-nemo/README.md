...
`speech-recognizer-nemo` では、固有名詞補強まわりの単体テストを `unittest` ベースで管理します。現時点では pytest 専用機能を前提にせず、作業環境標準の `uv run python -m unittest` を採用します。
...
- 確定結果を `voice_log_dir` 付きで出力すると、`<session>/<speech_id>_<timestamp>.json` と同じ並びで `.trace.json` が保存されます。
