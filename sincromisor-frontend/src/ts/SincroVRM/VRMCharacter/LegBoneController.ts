...
export class LegBoneController {
    ...
    update(elapsedSeconds: number): void {
        const sway = sineWave(elapsedSeconds, CHARACTER_IDLE_MOTION_CONFIG.legs.swayPeriodSeconds, Math.PI / 4);

        this.getNode('leftUpperLeg')?.rotation.set(MathUtils.degToRad(-3), MathUtils.degToRad(-5), MathUtils.degToRad(-3));
