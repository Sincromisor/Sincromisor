...
- `task.md`、未完了の依存、関連コードを現在の HEAD で確認する。変更経路と外部入力の供給元・消費先をたどる。既存の正本から一意に決まり、公開契約・責務・受け入れ条件を変えない不足は `AUTO_FIX` として実装担当へ渡し、根拠と補完内容を `impl.md` に記録する。複数の妥当な選択肢、公開設定や外部境界の供給方式、責務分担を決める必要がある場合だけ実装せず、タスク改訂が必要な理由を報告する。
