...
対話セッション上で回せる。Claude / Codex の両方がスキル・
サブエージェント・フックをネイティブ実装しているためで、`.claude/` を**単一ソース**とし、
...
上流キットとの差分は [.agents/CUSTOMIZATIONS.md](.agents/CUSTOMIZATIONS.md) に記録する。

生成する `.codex/agents/*.toml` は3体、`.agents/skills/*/SKILL.md` は2本。レビュー担当と評価担当は、
ユーザーの明示要求または高リスク変更の場合だけ起動する。通常変更は親Codexが現在のワークツリーで直接実装し、
変更範囲に対応する確認と1コミットで完了する。

## 前提

- **Codex CLI（>= 0.137。`multi_agent` / `hooks` が安定版）**: スキル（`.agents/skills/`）と
  サブエージェント（`.codex/agents/*.toml`）、SubagentStop フック（`.codex/hooks.json`）を使います。
  これらは `.claude/` を**単一ソース**に `gen:codex` で生成します（下記「生成」）。
- 共通の前提（**Git** / **Node.js（>=18）または Bun**）と Sincromisor 固有のタスクの作業手順は
...
生成元（コマンド定義・エージェント定義）を**改名・削除**したときに残る古い生成物（生成元のない生成物）は、`gen:codex` 実行時に
**自動整理（削除）**される（空になったスキルディレクトリも除去）。`gen:codex:check` は生成元のない生成物が
残っていれば**パスを列挙して exit 1**する。検出・削除の対象は**生成印（先頭コメント）を持つ
ファイルに限定**され、印の無い手書きファイルは触らない。
...
| Claude                                                     | Codex                                                    | 生成のしかた                                                                                         |
| ---------------------------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `.claude/commands/*.md`（スラッシュコマンド）              | `.agents/skills/<name>/SKILL.md`（スキル）               | 冒頭のメタデータ → `name`/`description`、本文の相対リンク深さを補正                                  |
| `.claude/agents/*.md`（サブエージェント）                  | `.codex/agents/*.toml`                                   | 本文を `developer_instructions` に、`tools:` → `sandbox_mode`、`model:` → `codexGen.modelMap` で変換 |
| `.claude/settings.json` の PostToolUse(Task\|Agent) フック | `.codex/hooks.json` の PostToolUse(`close_agent`) フック | 同一スクリプト `logAgentRun.mjs` を指す（Codex/Claude 両形式を防御的に解釈）。対話 TUI 前提          |
| `CLAUDE.md` / `AGENTS.md`                                  | `AGENTS.md`（Codex がネイティブに読む）                  | 変換不要                                                                                             |
| `scripts/**`（タスク管理・検査・評価）                     | そのまま                                                 | ランタイム非依存（Node/Bun）。変換不要                                                               |
...
- **ツール制限の粒度**: Claude の `tools:`（例: レビュー担当は Edit/Bash を持たない）に当たる
  ...
  （生成時に各 TOML へ短い前文を付与）。impl-evaluator は実装ワークツリーのコミット済み差分を
  変更せず評価する。
- **サブエージェントの自動起動なし**: Codex はサブエージェントを自動起動しない（親の明示指示で
  起動）。`run-task` スキル本文が必要な場合だけ明示起動する。スキルが
  説明マッチで暗黙起動され得る点は Claude のスラッシュコマンドと異なる。Codex は
  サブエージェントを multi_agent ツール（`spawn_agent` / `wait_agent` / `close_agent`）として
  実行する。
- **メトリクスフックは対話 TUI 前提（可能な範囲で実行）**: プロジェクトスコープのフックは**対話 TUI でのみ
  発火し、非対話 `codex exec` では発火しない**ことを 0.139.0 で確認した（`--dangerously-bypass-hook-trust`
    - プロジェクト信頼設定済みでも未発火）。生成する `.codex/hooks.json` はサブエージェント完了
      （`close_agent`）を `PostToolUse` で捕捉する（Claude の `Task|Agent` PostToolUse と同じ設計）。
      `close_agent` ペイロードの実フィールド名（利用 / 継続時間）は対話セッションで 1 度確認し、
