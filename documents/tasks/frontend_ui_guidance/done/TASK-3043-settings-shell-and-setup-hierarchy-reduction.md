...
- 2026-05-07: `SettingsSectionCard` の title / description を任意化し、ページ見出しとカード見出しが重なる箇所ではカードヘッダーを表示しない構造へ整理した。
- 2026-05-07: 常設設定パネルと初回セットアップの音声ページを `マイク補正` に寄せ、説明文はページ説明の1箇所だけにした。
- 2026-05-07: 初回セットアップの開始ボタン既定説明と VRM 差し替え説明を削り、接続ページは `接続状態` に絞った。
- 2026-05-07: `cd sincromisor-frontend && npm run build` で確認済み。
