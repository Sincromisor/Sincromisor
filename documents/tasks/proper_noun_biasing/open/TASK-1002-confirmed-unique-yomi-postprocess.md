...
2. 形態素列を基準に、連続する境界上でのみ最長一致を評価する。
3. 読みが一意な辞書語だけを自動補正対象にする。
4. `SpeechRecognizerNemoWorker` で confirmed 時のみ補正を呼び、補正後 result を下流へ返す。
5. 補正失敗時は元の認識結果を返す。
6. raw ASR result、補正ヒット件数、補正前後テキストをログまたは sidecar へ残す。
