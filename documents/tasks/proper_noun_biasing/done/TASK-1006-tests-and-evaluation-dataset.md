...
1. 辞書ローダのテストを追加する。
2. Python のテスト実行方式を確定し、必要なら依存を追加する。
3. 一意読み補正のテストを追加する。
4. 曖昧語の誤補正回避テストを追加する。
5. 非辞書文・一般会話文の負例セットを追加する。
6. 代表例をまとめた評価セットを用意する。
7. raw ASR result と補正 trace の確認手順を `documents/tasks` または関連 README に残す。
8. false positive 件数、非辞書文無変化率、confirmed レイテンシ増分を評価項目に含める。
