...
> **AGENTS.md との関係**: [AGENTS.md](../../AGENTS.md) は初動ガイドと関連文書へのリンクを保持する。タスク管理は [tasks/README.md](../../tasks/README.md)、コード構造は [code-structure.md](code-structure.md)、設計文書運用は [documents/design/documentation-guide.md](../design/documentation-guide.md) に従い、本書は Markdown 固有の横断ルールを定める。
...
2. **詳細はリンクで案内する** — エンドポイント、ペイロード、環境変数、運用手順などの説明を重複記載しない
...
- 設計文書の作成・更新・分割は [documents/design/documentation-guide.md](../design/documentation-guide.md) に従う。
...
- 文書の役割は「手順」「仕様」「規約」、参照の案内は「参照する」「従う」など、文脈に合う語で書く。「正本」は、複製や生成物との区別、情報が食い違う場合の優先元を明示する必要がある場合に限る。
...
- リポジトリ内リンクは相対リンクを使う。
- 同じ情報を複数文書にコピーしない。詳細を記載した文書へのリンクと、読む理由を 1 行で書く。
...
**理由**: 同じ情報が複数文書に分散すると、片方だけ更新されて実装判断を誤る。リンクは「重複の代替」として使う。
...
- 長いコードを Markdown に埋め込まない。文書から実装ファイルへリンクする。
...
- 仕様を図だけで説明しない。本文または契約文書から同じ情報へ辿れるようにする。
...
- **外部 URL だけで仕様の説明を済ませない** — 外部仕様へ依存する場合は、参照日と本プロジェクトで採用する範囲を書く。
