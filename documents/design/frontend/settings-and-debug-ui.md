...
- 右側パネルの状態と購読は `SincroAppRightToolPanelService` が保持し、Reactはアプリ制御の `debug` 操作窓口から利用する。窓口の組み立てでは状態取得や購読登録を実行しない。
- 起動前後の設定は同じ `DialogManager` の一括更新と設定スナップショットを使用する。操作不可の項目と機器ID以外の未指定値は変更せず、機器IDの `undefined` はブラウザー既定への復帰を表す。
- React側は設定値・操作可否・案内を個別に複製せず、`settingsStore` の同じスナップショットを参照する。一括適用後に更新を通知し、取り付け前の変更も初期取得へ反映する。解除時には設定購読も解放する。
- 有効な制御処理が変わると、Reactのイベント・設定購読は新制御処理へ切り替わり、初期状態から同期する。RTC停止ではアプリの外部イベント購読を解除せず、停止後も設定通知を受け取る。
