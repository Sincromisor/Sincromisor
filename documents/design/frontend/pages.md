...
- Developer pages:
    ...
    - page controller は `MotionDebugApp` を facade とし、VRM URL validation、camera / fixture source、TrackerRuntime bridge、replay、metrics / QA、window API binding、VRM scene / render cadence を `src/pages/motionDebug/motionDebug*Runtime.ts` と関連 module に分ける。公開 window API 名・引数・戻り値は `types.ts` の `MotionDebugApi` を正本にし、内部 module 境界の都合で増減させない。
