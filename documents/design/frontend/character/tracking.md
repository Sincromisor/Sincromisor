...
- `motion-debug`
    ...
    - 構造化 motion log recording は tracker callback と同じ `mediaTimeMs` で、motion-debug page 側の debug runtime snapshot から `frame.solver.phase6`、`frame.solver.phase7`、`frame.finalPose` を保存する。tracker runtime / Worker は Phase 6 solver snapshot、Phase 7 profile / calibration snapshot、VrmPoseComposer result、baseline metrics を所有しない。
    ...
    - solver layer は saved `frame.solver.phase6` と `frame.solver.phase7` を `{ phase6, phase7 }` の substatus として表示する。両方欠損した旧 log だけ外側 `solver` を `not_recorded` とし、片方が valid または invalid なら外側は `available` にする。旧 log の legacy `frame.solver.poseRetarget` は残すが、Phase 6 / Phase 7 solver layer の代替として再計算しない。
...
- motion evaluation log frame
    ...
    - 旧 log で `frame.reliability` が欠損している場合、replay viewer は `frame.poseSnapshot`、`frame.timestamp.mediaTimeMs`、`frame.video.width` / `height` から `createPoseReliabilityMap()` を再計算する。再計算にも使える `poseSnapshot` が無い場合だけ reliability layer は `not_recorded` になる。旧 log で `frame.solver.phase7` が欠損している場合は Phase 7 sublayer だけ `not_recorded` にし、log load 自体は失敗させない。
