...
- `TrackerRuntime`
    ...
    - main-thread fallback では effective target を face `<= 8fps`、pose `<= 4fps`、Hand ROI `<= 2fps`、Gesture `<= 2fps`、Face ROI `<= 3fps` に clamp し、`SincroTrackerWorkerStats.budget.degradation.state = "main-thread-low-fps"` として保存する。
    ...
    - `gesture-reduced-fps` は production Gesture optional pass の `gestureFps` を `max(1, floor(profile.cadence.gestureFps / 2))` に下げる。`optional-pass-reduced-fps` は Hand / Face ROI cadence を半減し、`pose-reduced-fps` は Pose cadence を `max(2, floor(profile.cadence.poseFps / 2))` に下げる。Face full-frame cadence は維持する。
    ...
    - `face-only` は既存 `degradePoseToFaceOnly()` 経路を使い、Pose / Hand / Gesture を止めて full-frame Face tracking を継続する。`comfortable-idle` は camera / Face tracking を止めず、Pose / Hand / Gesture / Face ROI を止めて Pose fallback、Hand lost snapshot、Gesture lost snapshot を出す。どちらの stage でも `latestPoseSnapshot` は clear し、ROI / Gesture optional pass が古い Pose snapshot を無期限に fresh 扱いしないようにする。comfortable pose の実際の blend は tracker runtime ではなく Temporal / MotionSolver / VrmPoseComposer 側の責務に残す。
    ...
    - Hand cadence は既定 `4fps`、指定範囲 `1..8fps` とする。Gesture cadence は performance profile の `gestureFps` を既定とし、指定範囲 `1..8fps` に clamp する。Gesture optional pass は同一 frame で Hand が実行された場合だけ走り、Hand tracker の左右 assignment を正本にして raw label を side snapshot へ正規化する。`poseOptions.faceRoi?.enabled === true` の場合だけ Face ROI を有効にし、Face ROI cadence は既定 `6fps`、指定範囲 `1..12fps` とする。どちらも `SincroPoseMotionSnapshot.lastUpdatedAtMs` が `mediaTimeMs - lastUpdatedAtMs > 250` の場合は `pose_stale_for_roi` として skip し、frame count だけで fresh 判定しない。full-frame Face cadence は従来どおり `DEFAULT_TARGET_INFERENCE_FPS` を正本にする。
    - production `sincro` の `startSincroFaceTracking()` は `enableSincroPoseTracking()` が true のときだけ `poseOptions.hand.enabled`、`poseOptions.gesture.enabled`、`poseOptions.faceRoi.enabled` を true で渡し、`onHandMotion` / `onGestureMotion` を observe-only pipeline / Debug Console summary へ接続する。Pose tracking が disabled の場合は Hand / Gesture / Face ROI も起動しない。
    - production Debug Console は Hand availability、source、ROI warning、openness、confidence と、Gesture availability、左右 top label、confidence、source、warning、inferenceFps の低頻度 summary だけを表示する。MediaPipe raw landmark、crop object、Hand wrist 座標、Gesture raw category list、handedness raw object は常時 snapshot に入れない。
    ...
    - Worker stats は optional `effectiveHandFps`、`effectiveGestureFps`、`effectiveFaceRoiFps`、`roi` を持つ。`roi` は pause state、fallbackCount、skippedFrames、consecutiveOverBudgetFrames、ROI reason code を保持し、既存 `effectiveFaceFps` / `effectivePoseFps` の意味は変えない。
    ...
    - tracker duration は `performance.now()` の単一 clock で測る。Worker の `workerTimeMs` は `detect()` entry（初回 `initialize()` より前）から result 組み立て直前までを含み、transport / round-trip は含めない。main-thread の `mainThreadDetectTimeMs` は frame callback 内の detect 開始から optional pass と stats 合成直前までを含む。`gestureInferenceTimeMs` は Gesture pass 実行時だけ保存し、lost result でも実測値を保持する。
...
- `SincroHandTracker`
    ...
    - Hand wrist は palm / finger reliability 材料であり、`SincroPoseMotionSnapshot.leftArm/rightArm.targets.wrist` を上書きしない。production arm IK target の primary は `TemporalUpperBodyState` 由来の body-local / scalar arm state であり、Hand wrist は腕 target の主入力にしない。
...
- `SincroGestureTracker`
    ...
    - Gesture optional pass は Pose enabled かつ Hand tracking enabled の場合だけ有効にし、同一 frame で Hand snapshot が検出済みのときだけ `recognizeForVideo()` を実行する。Hand snapshot 自体には gesture label を混ぜない。
    - snapshot は `SincroGestureMotionSnapshot`、左右 `SincroGestureSideSnapshot` の plain object に固定し、MediaPipe raw result、raw category list、landmark、crop object、ImageBitmap / VideoFrame / class instance を保存しない。
    - 1 hand に複数 category が返った場合は finite score 最大を top label にし、同 score tie は `categoryName` 昇順で決める。confidence は `0..1` に clamp し、category 欠損または non-finite だけの場合は side を `source: "lost"` にする。
    - Gesture handedness と Hand side assignment が食い違う場合は Hand side assignment を正本にし、side snapshot に `handedness_mismatch` warning を残す。Gesture handedness だけで left / right は入れ替えない。
    - GestureRecognizer の未ロード、初期化失敗、推論例外、Pose disabled / Hand disabled / `roi-hand-paused` / face-only / comfortable-idle は lost gesture snapshot と warning に落とし、Face / Pose / Hand 経路は継続する。
- Retargeters
...
- `motion-debug`
    ...
    - 構造化 motion log recording は MediaPipe raw result serializer が plain JSON 化できた slot だけを optional `frame.mediapipe` に保存する。保存対象は `pose`、`hand`、`face`、`gesture` と `timing.mediaTimeMs/videoWidth/videoHeight` で、MPMask、ImageBitmap、VideoFrame、crop canvas、MediaPipe task instance、landmark object prototype は保存しない。serializer 未対応の slot は省略し、空 object を記録済み raw result として扱わない。
    - manifest `build.packageVersions` は `sincromisor-frontend` と `@mediapipe/tasks-vision` の取得可能な version を保持し、取得不能な値は `"unknown"` とする。`build.configHash` は performance profile と retarget config から作る deterministic hash で、固定値や空 object にはしない。
    ...
    - solver layer は saved `frame.solver.phase6`、`frame.solver.phase7`、`frame.solver.phase9` を `{ phase6, phase7, phase9 }` の substatus として表示する。3 件すべて欠損した旧 log だけ外側 `solver` を `not_recorded` とし、いずれかが valid または invalid なら外側は `available` にする。旧 log の legacy `frame.solver.poseRetarget` は残すが、Phase 6 / Phase 7 / Phase 9 solver layer の代替として再計算しない。旧 log で `frame.solver.phase6.arms.<side>.source` が無い場合は `primarySource: "pose-snapshot-fallback"` 相当として表示し、schemaVersion は `sincro.phase6-solver.v1` のまま受理する。旧 log で `frame.solver.phase9` が無い場合は `phase9.status = "not_recorded"`、schema invalid は `phase9.status = "invalid"` とする。
    ...
    - replay 中は `TrackerRuntime.startFaceTracking()` を呼ばず、live camera / video fixture runtime と camera track を停止してから進める。video asset を読み直して MediaPipe runtime を再実行する video re-inference replay は対象外である。
    - `mediapipe-raw-result` mode は `frame.mediapipe` を raw replay schema で parse し、caller-provided `applyRawResult` が Pose / Hand / Face / Gesture の既存 normalizer 境界へ渡す。`frame.mediapipe` 欠損は `missing_mediapipe_raw_result` とし、`pose-snapshot` へ暗黙 fallback しない。schema 違反は `parse_error` とし、失敗 slot を error message / detail に残す。`applyRawResult` 未指定時だけ `unsupported_mode` を返す。
...
- `TrackerRuntimePerformanceProfile`
    ...
    - cadence は Face / Pose / Hand / Face ROI / Gesture の target fps default である。`TrackerRuntime.startFaceTracking()` は明示 `targetInferenceFps`、`poseOptions.targetInferenceFps`、`poseOptions.hand.targetInferenceFps`、`poseOptions.gesture.targetInferenceFps`、`poseOptions.faceRoi.targetInferenceFps` がある場合は明示値を優先し、未指定 field だけ profile cadence を使う。
    - profile 固定値:
...
- `PoseReliabilityEstimator`
    ...
    - `gesture` slot は optional `GestureIntentObservation` がある場合だけ `source: "gesture"` とし、top label confidence、Hand side assignment、Hand ROI、camera quality、temporal の最小値から `finalWeight` を作る。`stableDurationMs` は同じ normalized side + label が confidence `>= 0.70` で連続した時間だけ加算し、label / side 変更、confidence 低下、media time 逆行、previous 欠損で `0` に戻す。temporal score は `clamp(stableDurationMs / 160, 0, 1)` とし、valid な 0〜159ms は `source: "gesture"` / `unstable_observation`、160ms 以上は reason なしとする。Gesture 欠損だけを neutral / `no_observation` にする。
...
- motion evaluation log frame
    ...
    - `frame.solver.phase6` は motion-debug page 側で保存する Phase 6 solver snapshot であり、`profile.schemaVersion`、finite number だけの measurements、左右 arm の `source`、IK state / constraint reason code を確認するための developer-visible slot である。`source.primarySource` は `"temporal"` または `"pose-snapshot-fallback"`、fallback reason は `temporal_input_missing`、`avatar_profile_missing`、`temporal_arm_lost`、`invalid_temporal_arm`、`ik_solver_missing` のいずれかに固定する。MediaPipe raw result や tracker worker stats はこの slot に入れない。
    ...
    - 旧 log で `frame.reliability` が欠損している場合、replay viewer は `frame.poseSnapshot`、`frame.timestamp.mediaTimeMs`、`frame.video.width` / `height` から `createPoseReliabilityMap()` を再計算する。この fallback は旧 log 互換の pose-only placeholder であり、保存されていない Hand / Face / Gesture 観測を replay 時に捏造しない。saved reliability がある場合、reliability layer は `gesture.source`、`finalWeight`、`confidence`、`stableDurationMs`、warnings を含む `ReliabilityMap.gesture` を表示できる。Gesture raw category list や handedness raw object は `frame.mediapipe.gesture` の raw replay slot にだけ留め、reliability layer へ重複保存しない。再計算にも使える `poseSnapshot` が無い場合だけ reliability layer は `not_recorded` になる。旧 log で `frame.intent` / `frame.solver.phase7` / `frame.solver.phase9` が欠損している場合は該当 sublayer だけ `not_recorded` にし、log load 自体は失敗させない。
    ...
    - `frame.metrics.tracker` は optional `gestureInferenceTimeMs` と total 用の既存 field（Worker は `workerTimeMs`、main-thread は `mainThreadDetectTimeMs`）を保存する。`sincro.motion-debug-log.v1` は維持し、旧 log の欠損は有効とする。不正な負値・非有限値は duration parser が field 単位で除外して warning を返し、同じ frame の他 field や log 全体は捨てない。
    ...
    - ordered degradation policy snapshot は `frame.metrics.tracker.degradationPolicy` に保存する。motion-debug viewer の metrics layer は `degradationPolicy.stage`、`recovering`、`reasonCodes`、`effectiveCadence` と active runtime performance profile を developer-visible JSON として表示する。camera resolution 再交渉はこの policy snapshot の予約情報に留め、Gesture optional pass は production runtime の `effectiveGestureFps` として実適用する。
...
- motion metrics input boundary
    ...
    - `left-arm-occlusion-recovery` / `right-arm-occlusion-recovery` は対象腕だけに tracked 10 frame 以上、lost 5 frame 以上、recovering 2 frame 以上、再 tracked 10 frame 以上を発生させ、非対象腕を全 frame tracked に保つ deterministic QA protocol とする。両 fixture は recovery jump 18deg 以下（8deg 超は既知 WARN）、solver elbow flip reject 2 以下、final pose angular velocity clamp 3 以下、owned bone conflict 0 を gate とする。
    - Phase 5 temporal metrics は `temporalPredictedArmFrameCount`、`temporalRecoveringArmFrameCount`、`temporalLostArmDurationMs`、`temporalMaxRecoveryJumpDegEquivalent`、`temporalNeutralWristJitter` の 5 key を持つ。Phase 6 metrics は `solverElbowFlipRejectCount`、`solverReachClampOccupancy`、`solverExcessReachRatioP95`、`solverPoleUncertainFrameCount`、`finalPoseAngularVelocityClampCount`、`finalPoseOwnedBoneConflictCount` を追加する。reach p95 は bridge clamp 前の要求と solver 最終 target の差を全左右 arm-frame から nearest-rank 法で求め、旧 log または部分記録は利用しない。temporal primary への production 切替時は P0 replay fixture で neutral jitter、elbow flip count、recovery jump、reach clamp occupancy を pose-snapshot fallback baseline と比較し、regression が無いことを task artifact / `impl.md` に保存する。Phase 9 metrics は `gestureFlickerCount`、`semanticFallbackFrameCount`、`intentCooldownSuppressionCount`、`intentInvalidFrameCount` を追加する。Phase 10 metrics は `trackerBudgetOverrunFrameCount`、`trackerDroppedFrameCount`、`degradationStageFrameCount`、`degradationRecoveryFrameCount`、`roiPausedFrameCount` を追加し、すべて `unit: "count"`、`direction: "lower_is_better"` とする。すべて `MotionMetricResult.value: number | null` の単一数値 metric とし、invalid / missing input は `not_available` にする。
...
`motion.md` の production application gate は、tracker 由来の Hand / Face ROI、degradation、camera quality を入力条件として読む。tracking layer は VRM 適用可否を直接決めず、artifact と metric status で gate を止める材料を出す。

| input                      | gate impact                                                                                                                                                                                                                                                   | required artifacts                                                                                                                   | required metrics status                                                                                                                                                                                                                   | required manual verification                                                                                                                | rollback condition                                                                                                                                                                                                           |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hand ROI                   | arm flag 以降の gate では、Hand ROI が `pose_stale_for_roi`、`hand_roi_paused`、`roi_missing`、`roi_inconsistent` を区別して保存されていることを entry criteria にする。Hand wrist は palm / finger reliability 材料であり、腕 IK target の主入力にはしない。 | `frame.hand`、`SincroRoiObservation`、`frame.metrics.tracker.roi`、Debug Console Hand summary。                                      | `roiPausedFrameCount` が pass または既知理由付き warn、Hand reliability が `not_available_in_pose_snapshot` と `roi_missing` を同じ欠損へ重複付与しないこと。                                                                             | 片手 ROI lost、両手 ROI invalid full-frame fallback、hand pause、recovery 後の side assignment を確認する。                                 | Hand ROI 欠損で production callback が例外になる、Hand wrist が Pose wrist IK target を上書きする、または stale ROI が fresh として使われる場合は Hand ROI 接続を observe-only に戻す。                                      |
| Face ROI                   | observe-only / dry-run gate では、Face ROI metadata が head reliability の説明材料として保存され、Face retarget の既存入力値を置き換えないことを entry criteria にする。                                                                                      | `SincroFaceMotionSnapshot.roi`、`source`、`warnings`、`frame.metrics.tracker.roi`、Face ROI pause warning。                          | `trackingLossDurationMs` と head reliability が Face ROI pause / fallback を説明でき、Face center consistency を ReliabilityMap 側で再計算していないこと。                                                                                | Pose stale 中、Face ROI no-face、full-frame fallback、face-only fallback 中も Face cadence が Pose cadence に引きずられないことを確認する。 | Face ROI 失敗で full-frame Face が止まる、Face retarget が ROI confidence だけで無効化される、または crop-local landmark が snapshot に漏れる場合は Face ROI を disabled に戻す。                                            |
| ordered degradation policy | dry-run 以降の gate では、`degradationPolicy.stage` と ROI pause state が保存され、reduced fps / ROI pause / face-only / comfortable-idle が metric に反映されていることを entry criteria にする。                                                            | `frame.metrics.tracker.budget`、`frame.metrics.tracker.degradationPolicy`、`frame.metrics.tracker.roi`、active performance profile。 | `trackerBudgetOverrunFrameCount`、`trackerDroppedFrameCount`、`degradationStageFrameCount`、`degradationRecoveryFrameCount`、`roiPausedFrameCount` が pass または原因説明付き warn で、旧 log 欠損は推測せず `not_available` にすること。 | Worker fallback、main-thread-low-fps、ROI over-budget、face-only、comfortable-idle、recovery を motion-debug metrics layer で確認する。     | degradation stage が `"full"` 以外のまま戻らない、`ignorePerformanceFallback` が reduced fps / ROI pause stats まで抑制する、または comfortable-idle が古い Pose ROI を fresh 扱いする場合は production 適用 gate を閉じる。 |
| camera quality             | observe-only と初期 calibration gate では、camera quality が reliability / calibration の入力説明として保存され、raw device identifier を保持しないことを entry criteria にする。                                                                             | `frame.metrics.cameraQuality`、manifest camera settings、`CameraQualityScore` guide reason、initial calibration step status。        | camera component 欠損は該当 check だけ skipped とし、`CameraQualityScore.overall.status` が retry / failed の fixture は motion metric pass だけで次段へ進めないこと。                                                                    | low resolution、hands out of frame、motion blur、border risk、camera stop / restart で guide reason と retry status を確認する。            | raw `deviceId` / `groupId` / `label` が保存される、camera quality retry が無視されて calibration が pass 扱いになる、または source `none` で score を捏造する場合は gate を閉じる。                                          |
