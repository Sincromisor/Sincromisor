...
- `PoseReliabilityEstimator`
    ...
    - `gesture` slot は optional `GestureIntentObservation` がある場合だけ `source: "gesture"` とし、top label confidence、Hand side assignment、Hand ROI、camera quality の最小値から `finalWeight` を作る。`stableDurationMs` は同じ normalized side + label が confidence `>= 0.70` で連続した時間だけ加算し、label change、side missing、confidence 低下、media time 逆行で `0` に戻す。`stableDurationMs < 160` の間は `finalWeight` を `0.5` 以下に cap する。
