...
- `motion-debug`
    ...
    - 構造化 motion log recording は tracker callback と同じ `mediaTimeMs` で、motion-debug page 側の debug runtime snapshot から `frame.solver.phase6` と `frame.finalPose` を保存する。tracker runtime / Worker は Phase 6 solver snapshot、VrmPoseComposer result、baseline metrics を所有しない。
    ...
    - solver / finalPose layer は saved `frame.solver.phase6` / `frame.finalPose` を parse して `available`、欠損時は `not_recorded`、schema 違反時は `invalid` として表示する。旧 log の legacy `frame.solver.poseRetarget` は残すが、Phase 6 solver layer の代替として再計算しない。
