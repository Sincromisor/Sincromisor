...
- `SincroPoseRetargetFrame`
    ...
    - 3D IK は `SincroPoseTargetPointSnapshot.world` の normalized target を入力候補にする。MediaPipe world 座標から VRM rig scale / bone length / handedness へ変換する処理は tracker ではなく solver 側で行う。
