...
- motion metrics
    ...
    - Phase 6 arm の optional `reach` は bridge clamp 前の肩 local wrist 長を avatar arm length で割った `requestedReachRatio`、solver 最終 target の同じ比率 `appliedReachRatio`、両者の正の差 `excessReachRatio`、単一 ownership `clampedBy` を保存する。bridge と solver の両方が clamp した場合は `solver` を優先する。旧 log の欠損は parse できるが、`solverExcessReachRatioP95` は全 arm-frame に finite 診断が揃わない限り `reach_diagnostics_not_recorded` とする。
