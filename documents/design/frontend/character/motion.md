...
- `src/character/reliability`
    ...
    - Phase 8 以降の `PoseReliabilityEstimator` は `SincroPoseMotionSnapshot` と optional `SincroHandMotionSnapshot` / `SincroFaceMotionSnapshot` / `GestureIntentObservation` / `CameraQualityScore`、optional `previous.pose` / `previous.mediaTimeMs` / `previous.reliability`、caller 指定の `mediaTimeMs`、`video` size から `ReliabilityMap` を作る pure function とする。Hand / Face / Gesture 入力が省略された旧経路では該当 slot は pose-only placeholder を維持し、入力がある frame だけ `source: "hand"` / `"face"` / `"gesture"` の reliability を埋める。
    ...
    - Gesture reliability は `gestureReliabilityEstimator.ts` が `GestureIntentObservation`、Hand snapshot、前回 `ReliabilityMap.gesture`、CameraQualityScore、caller 指定 `mediaTimeMs` から作る。`components.tracking` は左右 top label confidence の最大値、`side` は Hand side assignment との整合、`roi` は Hand ROI metadata、`cameraQuality` は既存 camera component を読む。`temporal` は `clamp(stableDurationMs / 160, 0, 1)` で、valid な 0〜159ms は `unstable_observation`、160ms 以上は reason なし、Gesture 欠損だけ `no_observation` とする。`finalWeight` は5 component の最小値で、別建て cap は持たない。schemaVersion は v1 を維持し、旧 log の temporal score 0 / `no_observation` も parse する。
...
- `motion-debug`
    ...
    - Gesture / Hand / Face ROI は optional lower fps pass であり、ordered degradation policy v1 では `"gesture-reduced-fps" -> "optional-pass-reduced-fps" -> "roi-hand-paused" -> "pose-reduced-fps" -> "face-only" -> "comfortable-idle"` の順で段階的に退避する。pause 中も Gesture lost snapshot、Hand lost snapshot、full-frame Face snapshot は更新されるため、motion-debug / reliability は stale、lost、pause を区別できる。
...
- `MotionIntentState`
    ...
    - production observe-only pipeline は `SincroGestureMotionSnapshot` を直接 MotionIntent へ渡さず、`{ left?: { label; confidence }; right?: ... }` の `GestureIntentObservation` へ正規化してから `ReliabilityMap.gesture` と `MotionIntentEstimator.update({ gesture })` に渡す。`ReliabilityMap.gesture` は valid observation がある場合 `source: "gesture"` とし、unknown raw label でも semantic intent への昇格とは分けて保存する。Gesture optional pass が skip / lost の場合だけ `source: "neutral"` placeholder を返す。
    ...
    - confidence gate は ReliabilityMap がある場合 `ReliabilityMap.gesture.finalWeight >= 0.70`、該当 hand part `>= 0.60`、finger part `>= 0.45` を既定値とする。ReliabilityMap 欠損時は legacy / test 入力として gesture confidence と hand side confidence を fallback に使う。`MotionIntentEstimatorConfig.thresholds` 指定時だけ既定値を override する。
    ...
    - Gesture Recognizer は production optional pass として初期化済みであり、reliability 実観測接続は `ReliabilityMap.gesture` に閉じる。authored semantic clip asset と `VRMCharacterManager.update()` の適用順序変更は後続 task に残す。AnimationMixer を使う場合も semantic clip 再生は staging に留め、最終的には pose delta を `VrmPoseComposer` の semantic layer として渡す。
- `TemporalUpperBodyState` → arm IK bridge
    ...
    - `bodyLocalWrist` がある場合は主入力とし、body-local absolute tuple から `sideSign = left ? -1 : 1`、`shoulderLocal = [sideSign * shoulderWidth * 0.5, 0, 0]` を再構成し、`relative = bodyLocalWrist - shoulderLocal` を作る。body-local tuple は tracker の torso-normalized 座標で avatar meter ではないため、`relative` に lateral / vertical / depth scale を適用した方向を正規化し、長さは `reach * avatarArmLength * defaultReachScale` から与える。`bodyLocalElbow` がある場合の `elbowPole` は従来どおり肩相対方向へ変換する。
...
- Initial calibration
    ...
    - production settings の retry controller は idle / active / cancelled を sessionId 付きで管理する。active session だけ record / retry / cancel を受理し、stale id、inactive、未記録 step は state を変更しない。retry cascade は precheck=全step、neutral=neutral/a_pose/hand_open、a_pose と hand_open=自身だけを削除する。ready step も明示操作なら再試行でき、`ready_without_hands` は character 開始を妨げず hand_open だけ任意再試行できる。
    - production Pose callback は observe-only pipeline の `ReliabilityMap`、optional camera quality / canonical state、`mediaTimeMs` を calibration bridge へ渡す。bridge は既存の pure step evaluator を呼び、active sessionId で結果を retry controller へ記録する。UI は controller の通知を購読して同じ結果を表示し、retry cascade で step entry が削除された場合は、その step の valid duration を 0 から再計測する。
    - camera stop、`sincro` mode 離脱、camera / VRM source 変更では lifecycle owner が active session id で cancel する。cancelled state は session data を保持せず、再開は必ず新しい sessionId の start から行う。
...
- motion evaluation log
    ...
    - baseline 用 tracker duration summary は既存 motion metric schema から分離する。Gesture p95 は `gestureInferenceTimeMs` がある実行 frame だけ、total p95 は `tracker.mode` に従い Worker の `workerTimeMs` または main-thread の `mainThreadDetectTimeMs` だけを母集団にし、初回 Worker initialize cost も除外しない。有限値を昇順にした nearest-rank `ceil(0.95 * n) - 1` を使い、0 件は `null`、旧 log の欠損は warning なしで省略する。
...
- motion metrics
    ...
    - Phase 6 arm の optional `reach` は bridge clamp 前の肩 local wrist 長を bridge scale の avatar arm length で割った `requestedReachRatio`、solver 最終 target 長を同じ bridge scale arm length で割った `appliedReachRatio`、両者の正の差 `excessReachRatio`、単一 ownership `clampedBy` を保存する。profile measurement と solver measurement が異なる場合も requested / applied の分母を混在させない。bridge と solver の両方が clamp した場合は `solver` を優先する。旧 log の欠損は parse できるが、`solverExcessReachRatioP95` は全 arm-frame に finite 診断が揃わない限り `reach_diagnostics_not_recorded` とする。
...
腕 IK target の production primary は `TemporalUpperBodyState` と `MinimalAvatarMotionProfile` から作る
temporal bridge 出力である。Temporal input / avatar profile / bridge / solver が欠損または invalid な場合だけ
`SincroPoseMotionSnapshot.leftArm/rightArm.targets` の pose-snapshot fallback を使い、Hand ROI / Hand wrist は
palm / finger reliability と ROI 観測の材料に限定する。
