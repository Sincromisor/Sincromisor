...
- `TemporalUpperBodyState`
    ...
    - `dtMs <= 0`、`dtMs > 250`、非 finite dt の frame は filter 内部状態を更新せず、前回 filtered 値を維持して velocity を `0` にする。lost frame も canonical の低信頼値を filter に投入せず、前回 filtered 値と state/meta だけを更新する。dropout 中の constant-velocity prediction、`recovering` blend、VRM pose smoothing は後続 task の責務に残す。
