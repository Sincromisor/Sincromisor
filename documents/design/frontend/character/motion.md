...
- `motion-debug`
    ...
    - `solver` layer は Phase 6 の `frame.solver.phase6` を正本にする。旧 log の `frame.solver.poseRetarget` / `frame.solver.poseRetargetRuntime` は保持するが、Phase 6 snapshot としては live recompute せず `not_recorded` にする。saved `phase6` は `parseMotionDebugPhase6SolverSnapshot()` で検証し、未知 schemaVersion、非 finite number、unknown enum、runtime object 風 value は `invalid` layer として表示する。
...
- `AvatarMotionProfile`
    ...
    - `SincroPoseRetargeter.attachVrm()` は完成版 `AvatarMotionProfile` を生成・保持し、`getAvatarMotionProfile()` は deep clone 済みの完成版 profile を返す。既存 Debug Console / motion-debug / Phase 6 composer / solver へ渡す箇所では minimal 互換変換を明示する。
