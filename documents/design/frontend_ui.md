...
- 状態表示:
  - 各設定ページの先頭に `現在の状態` カードを置き、接続状態、選択デバイス、反映タイミングなどを短く要約表示する
...
| 2026-04-19 | 設定UIを `上部ヘッダー + 左カテゴリナビ + 右詳細ペイン` へ再整理する方針を追記。主要カテゴリを `会話` / `入出力デバイス` / `音声` / `表示` / `接続` に再定義し、状態カード、接続操作の分離、起動前 dialog と開始後パネルの文言・情報設計共通化を明文化 |
| 2026-04-19 | `SettingsShell` を追加し、起動前 dialog と `SimpleVrmControlPanel` 系を共通のカテゴリナビシェルへ移行。接続操作と開始時オプションを `接続` ページへ集約し、dialog 側にも同じカテゴリ構成と状態カードを追加 |
| 2026-04-19 | `simple-vrm` を優先対象にカテゴリ再編を実施。`入出力デバイス` を新設し、`起動` カテゴリを `接続` へ吸収。設定シェルはサイドナビと本文開始位置を安定させ、非操作要素が押せるように見えない表現へ整理 |
