...
### 2.3 Discordライク設定シェル方針

- 対象ユーザー:
  - PCで音声通話や配信ツールに慣れた利用者を主対象とし、Discord の PC版設定画面に近い探索性を目指す
...
- 基本構造:
  ...
  - 右詳細ペインは `ページタイトル / 1文説明 / 状態サマリー / 設定セクション / フッター操作` の順で構成する
- カテゴリ設計:
  - 一般ユーザー向けの主要カテゴリは `会話`、`音声`、`表示`、`起動`、`接続` とする
...
- 状態表示:
  - 各設定ページの先頭に `現在の状態` カードを置き、接続状態、マイク状態、再生先状態などを要約表示する
  - 状態表現は色だけに依存せず、`接続済み`、`マイク入力あり` のような文言付き badge と短い詳細文を併記する
  - 行動が必要な状態は、対象ページ上で復帰導線とセットで表示する
- 操作導線:
...
- フォーム設計:
  ...
  - 音声デバイス選択は `現在の選択`、`一覧を更新`、`テスト`、`入力レベル` を同じページ内にまとめ、選択結果に確信を持てるUIとする
  - 項目量が多いカテゴリは、`入力デバイス`、`前処理`、`開始時のオプション` のようにセクション面を分けて整理する
  - ヘルプは小さな `?` を画面中に散らすより、項目直下の短い説明文または詳細展開に寄せる
- レイアウト/見た目:
  - カテゴリ単位で過剰なカード分割を行わず、`見出し + フォーム + 区切り線` を基本とする
  - 初回セットアップ dialog は `幅 960-1120px`、`高さ 620-780px` を目安とし、右本文の可読幅は `560-760px` を下回らないようにする
  - 開始後の右側設定パネルは `420-560px` 幅を目安とし、本文として読ませる領域を `320px` 未満にしない
  - 起動前 dialog と開始後の設定パネルは、レイアウト差があってもカテゴリ名、状態カード、項目順、文言方針をそろえる
...
- 要件一覧:
  ...
  - 起動前設定 dialog と開始後の設定パネルは、Discordライクな `左カテゴリナビ + 右詳細ペイン` の情報設計を共有し、PC利用時に項目を探しやすいこと
  ...
  - 主要カテゴリは `会話`、`音声`、`表示`、`起動`、`接続` を基本とし、`詳細設定` と `開発者向け` を通常設定から分離すること
  - 各設定ページの先頭に、接続状態や入力可否などの要点を要約した状態カードを置けること
...
| 2026-04-19 | 設定UIを Discord ライクな `上部ヘッダー + 左カテゴリナビ + 右詳細ペイン` へ再整理する方針を追記。主要カテゴリを `会話` / `音声` / `表示` / `起動` / `接続` に再定義し、状態カード、接続操作の分離、起動前 dialog と開始後パネルの文言・情報設計共通化を明文化 |
| 2026-04-19 | `SettingsShell` を追加し、起動前 dialog と `SimpleVrmControlPanel` 系を共通のカテゴリナビシェルへ移行。接続操作を `接続` ページへ集約し、dialog 側にも `起動` / `接続` ページと状態カードを追加 |
